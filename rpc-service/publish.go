...
func main() {
	...
	// 使用flag包定义命令行参数
	flag.StringVar(&ip, "i", strings.Split(config.PUBLISH_SERVER_ADDR, ":")[0], "The IP address to listen on")
	flag.StringVar(&port, "p", strings.Split(config.PUBLISH_SERVER_ADDR, ":")[1], "The port number to listen on")
